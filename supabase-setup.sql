...
create table if not exists public.notifications (
  ...
  type text not null check (type in ('private_message', 'like', 'reply', 'media_created')),
...
begin
  ...
  if caller_role not in ('admin', 'superadmin') then raise exception 'No tienes permiso para cambiar rangos.'; end if;
  if new_role not in ('member', 'admin') then raise exception 'Rango no válido.'; end if;
  if target_user_id = auth.uid() then raise exception 'No puedes cambiar tu propio rango.'; end if;
  select is_hidden into target_hidden from public.profiles where id = target_user_id;
  if target_hidden is null then raise exception 'El usuario no existe.'; end if;
  if target_hidden then raise exception 'No se puede modificar una cuenta oculta.'; end if;
...
begin
  insert into public.notifications (user_id, actor_id, type, target_type, target_id, excerpt)
  select profiles.id, new.user_id, 'media_created',
    case when tg_table_name = 'moments' then 'moment' else 'post' end,
    new.id, left(coalesce(new.caption, ''), 180)
...
create trigger notify_moment_created after insert on public.moments
...
create trigger notify_post_created after insert on public.profile_posts
  for each row execute procedure public.notify_media_created();
...
-- Logros del club. Los miembros pueden verlos y solo la administración puede
-- crearlos, asignarlos o eliminarlos.
...
create policy "admins create achievements" on public.achievements
  for insert to authenticated
  with check (created_by = auth.uid() and public.current_user_is_club_admin());
...
create policy "admins create achievement awards" on public.achievement_awards
  for insert to authenticated
  with check (awarded_by = auth.uid() and public.current_user_is_club_admin());
