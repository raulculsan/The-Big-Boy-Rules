...
-- Correo privado de recuperación. Solo la función account-email puede acceder.
...
create table if not exists public.account_emails (
  ...
  constraint account_emails_format_check check (
    char_length(email) between 6 and 254 and email = lower(email)
    and email ~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
    and email !~ '@bigboyrules\.local$'
  )
);
create unique index if not exists account_emails_email_unique on public.account_emails (lower(email));
...
create index if not exists account_email_codes_user_created_idx on public.account_email_codes (user_id, created_at desc);
create index if not exists account_email_codes_expiry_idx on public.account_email_codes (expires_at);
